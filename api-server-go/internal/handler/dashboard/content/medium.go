...
func appendMediumFullPaths(content map[string]interface{}) {
	...
	for rawKey, fullKey := range pairs {
		if path, ok := content[rawKey].(string); ok && path != "" {
			content[fullKey] = storage.DefaultStorage.GetURL(path)
			content[rawKey+"Url"] = storage.DefaultStorage.GetURL(path)
		}
	}

	// 处理其他可能的路径字段
	otherFields := []string{"path", "thumbPath", "coverPath"}
	for _, field := range otherFields {
		if path, ok := content[field].(string); ok && path != "" {
			content[field+"Url"] = storage.DefaultStorage.GetURL(path)
			content["full"+field[0:1]+field[1:]] = storage.DefaultStorage.GetURL(path)
		}
	}
}
