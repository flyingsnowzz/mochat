...
// appendMediumFullPaths 添加素材完整路径到 content 中
// 为素材内容的各个字段添加完整的访问路径
func appendMediumFullPaths(content gin.H) {
	if path, ok := content["path"].(string); ok && path != "" {
		content["fullPath"] = path
		content["url"] = path
	}
	if path, ok := content["thumbPath"].(string); ok && path != "" {
		content["fullThumbPath"] = path
		content["thumbUrl"] = path
	}
	if path, ok := content["coverPath"].(string); ok && path != "" {
		content["fullCoverPath"] = path
		content["coverUrl"] = path
	}
}
