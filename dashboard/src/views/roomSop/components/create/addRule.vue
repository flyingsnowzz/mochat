...
/**
 * 群SOP规则创建/编辑弹窗组件
 * 功能说明：SOP（Standard Operating Procedure）是标准操作程序，用于设置群聊跟进规则
 * 主要功能：
 * 1. 设置内容名称（仅内部可见）
 * 2. 设置发送时间：
 *    - 模式1：加入规则后 X小时Y分钟后提醒发送
 *    - 模式2：加入规则后 X天后，当日 HH:MM 提醒发送
 * 3. 设置发送内容：
 *    - 支持文字和图片两种类型
 *    - 可添加多个发送内容
 * 4. 编辑已有规则
 *
 * 业务场景：
 * - 当客户加入群聊后，系统在侧边栏快捷回复中智能提醒员工执行SOP规则
 * - 员工可按照规则内容，定时向群聊发送预设的消息
 *
 * 技术实现：
 * - 使用 a-modal 弹窗
 * - 使用 a-radio-group 选择发送时间模式
 * - 使用 a-input-number 设置时间参数
 * - 使用 a-time-picker 选择具体时间
 * - 使用 m-upload 组件上传图片
 * - 通过 $emit 向上级组件传递数据
 */
