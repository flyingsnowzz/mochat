<!--
 * 自动拉群管理页面
 * 功能说明：管理企业微信自动拉群规则，实现客户自动进群的功能
 * 主要功能：
 * 1. 自动拉群流程展示：通过多步骤说明自动拉群的工作原理和优势
 * 2. 群活码管理：创建、编辑、查看、下载群活码
 * 3. 多维度数据展示：二维码、使用成员、标签、客户数、群聊列表等
 * 4. 搜索功能：通过群活码名称搜索
 * 业务场景：
 * - 企业需要快速将大量客户自动拉入群聊，无需人工操作
 * - 突破企业微信官方只能选择5个群聊的限制
 * - 当群聊达到人数上限时，自动引导客户进入新群
 * 技术实现：
 * - 使用 a-table 展示群活码列表，支持分页和多选
 * - 使用 a-modal 弹窗展示群活码详情
 * - 使用 v-permission 指令控制按钮权限
 * - 调用 workRoomAutoPullList 和 autoPullShow API 获取数据
-->
<template>
