...
/**
 * 客户SOP规则创建/编辑弹窗组件
 * 功能说明：SOP（Standard Operating Procedure）是标准操作程序，用于设置客户跟进规则
 * 主要功能：
 * 1. 设置规则名称（仅内部可见）
 * 2. 设置发送时间（客户添加员工后多少分钟/天提醒）
 * 3. 设置发送内容（支持文字和图片）
 * 4. 支持添加多条消息内容
 * 5. 支持编辑已有规则
 *
 * 业务场景：
 * - 当新客户添加企业员工后，系统在侧边栏快捷回复中智能提醒员工执行SOP规则
 * - 员工可按照规则内容，定时向客户发送预设的消息
 *
 * 技术实现：
 * - 两种发送时间模式：立即提醒（小时后/分钟）/ 定时提醒（天后/具体时间）
 * - 消息内容支持文字和图片两种类型
 * - 通过 $emit 向上级组件传递数据
 */
