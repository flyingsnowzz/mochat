 ...
 *    - 客户标签：选择或创建标签
 * 2. 引流成员设置：
 *    - 类型选择：单人或多人模式（一旦创建不可修改）
 *    - 企业成员添加：设置不同时间段的负责成员
 *    - 特殊时期设置：设置节假日等特殊时期的成员
 *    - 员工添加上限：设置每个成员的添加好友上限
 *    - 备用员工：当成员达到上限时使用
 * 3. 欢迎语设置：
 *    - 扫码推送：开启或关闭欢迎语推送
 *    - 通用欢迎语：默认欢迎语
 ...
 *
 * 技术实现：
 * - 使用 Ant Design Vue 组件库构建表单和弹窗
 * - 使用 moment.js 处理时间和日期
 * - 集成自定义 upload 组件处理文件上传
 * - 支持树形选择器选择部门
 * - 多标签页管理不同类型的欢迎语
 * - 权限控制：使用 v-permission 指令控制按钮权限
 */
<template>
  <div class="wrapper">
    <a-card title="基础设置">
 ...
 * 4. 设置特殊时期（节假日等）的备用成员
 * 5. 配置员工添加好友上限和备用员工
 * 6. 设置欢迎语（通用/周期/特殊时期三种模式）
 * 7. 支持素材管理（图片、图文、小程序）
 *
 * 技术实现：
 * - 使用 moment 处理时间
 * - 组件化：Department（部门选择）、upload（文件上传）
 * - 多种欢迎语模式：通用欢迎语、周期欢迎语（按周）、特殊时期欢迎语
 * - 引流成员时间槽管理，支持按时间段分配不同员工
 * - 素材选择弹窗，支持本地上传和素材库选择
 */
