    ...
    <a-card>
      <div class="table-page-search-wrapper">
        <a-form>
          <a-row :gutter="16">
            <a-col :lg="8">
              <a-form-item label="手机号码：" :labelCol="{ lg: { span: 7 } }" :wrapperCol="{ lg: { span: 14 } }">
                <a-input v-model="screenData.phone"></a-input>
              </a-form-item>
            </a-col>
            <a-col :lg="8">
              <a-form-item label="状态：" :labelCol="{ lg: { span: 7 } }" :wrapperCol="{ lg: { span: 14 } }">
            ...
            <a-col :lg="8">
              <a-form-item label="" :labelCol="{ lg: { span: 7 } }" :wrapperCol="{ lg: { span: 14 } }">
        ...
        <div class="alter">
          ...
          <span
            >已启用<span style="color: #4ba9ff">{{ usingNum }}</span> 项 未启用:<span style="color: #757e82">{{
              forbiddenNum
            }}</span
            >人</span
          >
          ...
          >
            ...
            <template slot="footer">
              ...
              <a-button @click="status" key="submit" type="primary"> 确定 </a-button>
            </template>
          </a-modal>
        </div>
      </div>
      ...
      <!--      添加弹窗-->
      <a-modal :visible="modalVisible" :maskClosable="false" @cancel="reset" title="基础信息">
        <a-form-model
          ref="ruleForm"
          :model="userData"
          :rules="rules"
        ...
        >
          ...
          <a-form-model-item label="手机号码：" prop="phone" style="position: relative">
            ...
            <span style="position: absolute; left: 5px; top: 20px; color: #a9a9a9"
              >输入的手机号需与企业微信中员工的手机号一致</span
            >
          </a-form-model-item>
        ...
        <template slot="footer">
          <a-button key="submit" :loading="btnLoading" type="primary" @click="addSubManagement"> 确定 </a-button>
          <a-button @click="reset" key="back"> 取消 </a-button>
        </template>
      </a-modal>
    </a-card>
    <!--    重置密码-->
      ...
      this.$refs.ruleForm.validate((valid) => {
        ...
        if (valid) {
          ...
          if (this.userId !== '') {
            ...
            addSubManagement(this.userData)
              .then((res) => {
                this.btnLoading = false
                this.modalVisible = false
                this.getTableData()
